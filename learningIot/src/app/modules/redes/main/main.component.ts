...
export class MainComponent implements OnInit {
    ...
    explain4:string[] = [
      "Incluye el archivo de encabezado de la biblioteca estándar de C para funciones de entrada y salida, permitiendo el uso de, por ejemplo, printf().",
      "Incorpora declaraciones para funciones de operaciones de internet, como inet_ntoa() que se utiliza más adelante.",
      "Incorpora funciones y estructuras que se usan para las consultas de base de datos de red, como gethostbyname().",
      "Declara el punto de entrada principal del programa. La ejecución comienza aquí.",
      "Inicializa un puntero llamado hostname que apunta a la cadena 'www.example.com'.",
      "Declara un puntero a una estructura hostent llamada he. Esta estructura se utilizará para almacenar información sobre el host que se busca.",
      "Declara un puntero doble a una estructura in_addr, que se utilizará para almacenar direcciones IP en formato legible.",
      "La función gethostbyname() busca el hostname en la base de datos de nombres de host. Si lo encuentra, asigna la información del host a he. Si no, he se establece en NULL. Si he es NULL (indicando un error en gethostbyname()), esta función imprime un mensaje de error relacionado con 'gethostbyname'.",
      "Si hay un error (he es NULL), el programa termina y devuelve un valor de 1, indicando un error.",
      "Cierra el bloque condicional if.",
      "Aquí se asigna a addr_list la lista de direcciones IP que se encontró con gethostbyname(). El tipo de datos se convierte para coincidir con la declaración anterior de addr_list.",
      "Este es el inicio de un bucle que iterará sobre cada dirección IP encontrada para el hostname.",
      "Imprime el nombre del host (hostname) junto con una de sus direcciones IP asociadas.",
      "La función inet_ntoa() convierte la dirección IP de formato binario a una cadena.",
      "Cierra el bucle for.",
      "Finaliza el programa y devuelve 0, indicando que todo salió bien.",
